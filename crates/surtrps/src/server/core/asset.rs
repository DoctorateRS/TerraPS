...
pub async fn get_file(Path(asset): Path<Asset>) -> Response {
    ...
    asset_list.entry(asset.hash.clone()).and_modify(|v| v.push(asset.name.clone())).or_insert(vec![asset.name.clone()]);
    ...
    if ASSET_CONFIG.dl_local {
        ...
        let builder = res
            ...
            .fold(Response::builder().status(res.status().as_u16()), |builder, (k, v)| builder.header(k, v));
