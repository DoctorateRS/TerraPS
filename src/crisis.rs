...
pub async fn crisis_v2_get_info() -> Json<Value> {
    ...
    let selected_crisis = config["crisisV2Config"]["selectedCrisis"]
        .as_str()
        .unwrap_or("cc2");
    Json(read_json(
        format!("{CRISIS_V2_JSON_BASE_PATH}{selected_crisis}.json").as_str(),
    ))
}
...
pub async fn crisis_v2_battle_finish(Json(payload): Json<Value>) -> Json<Value> {
    ...
    for slot in slots {
        ...
        let node_data = rune["info"]["mapDetailDataMap"][&map_id]["nodeDataMap"][&slot];
    }
    Json(json!({}))
}
