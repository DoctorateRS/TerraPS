...
async fn get_buffs(rlv2: &Value, stage_id: &str) -> Vec<Value> {
    ...
    let mut theme_buffs = match theme {
        "rogue_1" => read_json(&format!("./data/rlv2/themeBuffs/buffs_{theme}.json")),
        "rogue_2" => read_json(&format!("./data/rlv2/themeBuffs/buffs_{theme}.json")),
        "rogue_3" => read_json(&format!("./data/rlv2/themeBuffs/buffs_{theme}.json")),
        _ => json!([]),
    };
